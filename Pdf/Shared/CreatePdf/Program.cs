...
namespace CreatePdf
{
    class Program
    {
        ...
        Program()
        {
            ...
            _word.Info.Company = "ComponentOne";
        ...
        private string Quotes()
        {
			...
			{
				// literary quotations
				GetContent(authorsQuotes, 208);

				// 'no-topic' quotations
				GetContent(authorsQuotes, 195);
			}
			catch
			{
		...
		private void GetContent(List<string> authorsQuotes, int categotyId)
		{
            var content = string.Empty;
            var path = Path.Combine("Data", $"quote-{categotyId}.htm");

            // read resource from local file
            // there was a web resource at URL = $"http://quoteland.com:80/topic.asp?CATEGORY_ID={categotyId}"
